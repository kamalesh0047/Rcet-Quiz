...
app.get('/health', (_req, res) => res.json({ ok: true, ts: Date.now() }));app.use('/api/quizzes', quizRoutes);app.use('/api/games', gameRoutes);app.use('/api/results', resultRoutes);app.use(errorHandler);
